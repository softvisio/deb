...
export default class extends Command {
    ...
    async run () {
        ...
        res = childProcess.spawnSync( "git", [ "push", "--force", "--all" ], { "cwd": this.root, "stdio": "inherit" } );
        if ( res.status ) return result( 500 );

        res = childProcess.spawnSync( "git", [ "push", "--force", "--tags" ], { "cwd": this.root, "stdio": "inherit" } );
        if ( res.status ) return result( 500 );
    ...
    prune () {
        ...
        res = childProcess.spawnSync( "git", [ "filter-repo", "--analyze" ], { "cwd": this.root, "stdio": "inherit" } );
        if ( res.status ) return result( 500 );
        ...
        // .filter( line => line.startsWith( "dists/" ) ); // XXX

        if ( !deleted.length ) result( 200 );

        // XXX
        // --paths-from-file .git/filter-repo/analysis/path-deleted.txt
        res = childProcess.spawnSync( "git", [ "filter-repo", "--force", "--partial", "--invert-paths", ...deleted.map( path => `path=${ path }` ) ], { "cwd": this.root, "stdio": "inherit" } );
        if ( res.status ) return result( 500 );

        // git garbage collection
        res = childProcess.spawnSync( "git", [ "reflog", "expire", "--expire-unreachable=now", "--all" ], { "cwd": this.root, "stdio": "inherit" } );
        if ( res.status ) return result( 500 );

        res = childProcess.spawnSync( "git", [ "gc", "--prune=now", "--aggressive" ], { "cwd": this.root, "stdio": "inherit" } );
        if ( res.status ) return result( 500 );
